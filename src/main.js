...
const broomGrip=group(dog.body);
props.clean=group(broomGrip);for(const y of [.06,-.17])ball(props.clean,colors.cream,0,y,0,.085,.075,.085);box(props.clean,colors.wood,0,-.12,0,.035,1.02,.035);box(props.clean,colors.gold,0,-.65,0,.29,.23,.07);
...
function animate(){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.05);if(!paused){time+=dt;const state=director.tick(dt),id=state.walking?null:state.active?.id,t=state.elapsed;
 ...
 dog.body.rotation.set(0,0,0);dog.head.position.z=0;dog.head.rotation.set(0,Math.sin(time*.6)*.055,Math.sin(time*1.4)*.022);
 ...
 dog.arms.forEach((arm,i)=>{arm.scale.set(1,1,1);arm.children[0].position.x=i===0?-.035:.035;arm.rotation.set(state.walking?Math.sin(time*8+i*Math.PI)*.35:0,0,0);});
 ...
 if(id==='clean'){sweepPose(dog,props.clean,t);}
 if(id==='nap'){napPose(dog,time);}
